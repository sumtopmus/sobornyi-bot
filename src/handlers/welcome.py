...
async def about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    tools.clear_jobs(context, f'{WELCOME_TIMEOUT_JOB}:{incoming_message.from_user.id}')
    context.user_data['about'] = incoming_message.text
    message = f'Вітаємо тебе, {tools.mention(incoming_message.from_user)}!'
    # TODO: handle the case when incoming_message.message_thread_id is incorrect.
    welcome_message = await context.bot.sendMessage(
