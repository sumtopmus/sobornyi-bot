...
async def not_about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    message = f'{utils.mention(user)}, додай, будь ласка, до свого повідомлення теґ #about'
...
async def about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    if update.message:
        ...
        utils.clear_jobs(context.application, utils.MESSAGE_CLEANUP_JOB, incoming_message.id)
    ...
    if incoming_message.message_thread_id != settings.TOPICS['welcome']:
        try:
            ...
            await context.bot.sendMessage(
                chat_id=settings.CHAT_ID, message_thread_id=settings.TOPICS['welcome'],
                text=message)
    ...
    bot_message = await context.bot.sendMessage(
        chat_id=incoming_message.chat.id, message_thread_id=settings.TOPICS['welcome'],
        text=message, reply_to_message_id=reply_to_message_id)
    utils.add_message_cleanup_job(context.application, bot_message.id)
