...
def create_handlers() -> list:
    ...
    return [
        ConversationHandler(
            entry_points=[
                CallbackQueryHandler(
                    on_add_event, pattern="^" + State.EVENT_ADDING.name + "$"
                ),
                CallbackQueryHandler(
                    on_pick_event, pattern="^" + State.EVENT.name + ":"
                ),
            ],
            states={
                State.EVENT_MENU: [
                    CallbackQueryHandler(
                        on_edit_title,
                        pattern="^" + State.EVENT_EDITING_TITLE.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_emoji,
                        pattern="^" + State.EVENT_EDITING_EMOJI.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_description,
                        pattern="^" + State.EVENT_EDITING_DESCRIPTION.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_category,
                        pattern="^" + State.EVENT_EDITING_CATEGORY.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_occurrence,
                        pattern="^" + State.EVENT_EDITING_OCCURRENCE.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_datetime,
                        pattern="^" + State.EVENT_EDITING_DATETIME.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_venue,
                        pattern="^" + State.EVENT_EDITING_VENUE.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_location,
                        pattern="^" + State.EVENT_EDITING_LOCATION.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_url, pattern="^" + State.EVENT_EDITING_URL.name + "$"
                    ),
                    CallbackQueryHandler(
                        on_edit_image,
                        pattern="^" + State.EVENT_EDITING_IMAGE.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_preview, pattern="^" + State.EVENT_PREVIEW.name + "$"
                    ),
                    CallbackQueryHandler(
                        on_publish, pattern="^" + State.EVENT_PUBLISHING.name + "$"
                    ),
                    CallbackQueryHandler(
                        on_delete_event, pattern="^" + State.EVENT_DELETING.name + "$"
                    ),
                    CallbackQueryHandler(
                ...
                State.DATETIME_MENU: [
                    CallbackQueryHandler(
                        on_edit_date, pattern="^" + State.EVENT_EDITING_DATE.name + "$"
                    ),
                    CallbackQueryHandler(
                        on_edit_end_date,
                        pattern="^" + State.EVENT_EDITING_END_DATE.name + "$",
                    ),
                    CallbackQueryHandler(
                        on_edit_time, pattern="^" + State.EVENT_EDITING_TIME.name + "$"
                    ),
                    CallbackQueryHandler(
                        on_edit_end_time,
                        pattern="^" + State.EVENT_EDITING_END_TIME.name + "$",
                    ),
                ...
                State.EVENT_EDITING_CATEGORY: [
                    CallbackQueryHandler(
                        edit_category, pattern="^" + State.CATEGORY.name
                    ),
                ],
                State.EVENT_EDITING_OCCURRENCE: [
                    CallbackQueryHandler(
                        edit_occurrence, pattern="^" + State.OCCURRENCE.name
                    ),
                ],
                ...
                State.EVENT_DELETING_CONFIRMATION: [
                    CallbackQueryHandler(
                        delete_event,
                        pattern="^" + State.EVENT_DELETING_CONFIRMATION.name + "$",
                    ),
                ],
            },
            fallbacks=[
                ...
                CallbackQueryHandler(
                    calendar_menu, pattern="^" + State.CALENDAR_MENU.name + "$"
                ),
                CallbackQueryHandler(
                    event_menu, pattern="^" + State.EVENT_MENU.name + "$"
                ),
                CallbackQueryHandler(
                    datetime_menu, pattern="^" + State.DATETIME_MENU.name + "$"
                ),
...
async def on_edit_title(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    text = "Будь ласка, вкажіть назву заходу."
    await update.callback_query.edit_message_text(
        text, **construct_back_button(State.EVENT_MENU)
    )
...
async def edit_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    log("edit_time")
    context.user_data["current_event"].time = datetime.strptime(
...
async def edit_end_time(update: Update, context: ContextTypes.DEFAULT_TYPE) -> State:
    ...
    log("edit_end_time")
    context.user_data["current_event"].end_time = datetime.strptime(
        update.message.text, "%H:%M"
    ).time()
    return await datetime_menu(update, context)
...
async def on_delete_event(update: Update, context: CallbackContext) -> State:
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🗑️ Так", callback_data=State.EVENT_DELETING_CONFIRMATION.name
            ),
...
def construct_picker_keyboard(
    value, prefix: str, buttons: list, num_cols: int = 2
) -> InlineKeyboardMarkup:
    keyboard, row = [], []
    for button_text, button_value in buttons:
        row.append(
            InlineKeyboardButton(
                button_text + (" ✅" if value == button_value else ""),
