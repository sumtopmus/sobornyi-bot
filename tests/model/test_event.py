...
class TestEvent:
    ...
    def test_get_current_repr_past_date(self, mock_event):
        """Test that get_current_repr returns None for a completely past event."""
    ...
    def test_get_full_repr_end_date_different_months(self, mock_event):
        """Test get_full_repr when end_date is in a different month."""
        mock_event.date = date(2023, 1, 30)
        ...
        # Should include both month/day for end_date
        assert mock_event.end_date.strftime("%m/%d") in full_repr
        # Verify the start date is also included
        assert mock_event.date.strftime("%m/%d") in full_repr

    def test_get_full_repr_end_date_same_month(self, mock_event):
        """Test get_full_repr when end_date is in the same month."""
        mock_event.date = date(2023, 1, 1)
        ...
        # Should include only day for end_date
        assert mock_event.end_date.strftime("%d") in full_repr
        # Should not include month for end_date
        assert mock_event.end_date.strftime("%m/%d") not in full_repr
        # Verify the start date is included with month
        assert mock_event.date.strftime("%m/%d") in full_repr
    ...
    def test_get_full_repr_with_no_location_no_venue(self, mock_event):
        """Test get_full_repr with no location and no venue."""
        mock_event.location = None
        mock_event.venue = None
        assert mock_event.get_full_repr() is not None
        assert "📍" not in mock_event.get_full_repr()
